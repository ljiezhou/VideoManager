...
import com.ds.glide.GlideRequests
import com.ds.glide.IGlideModule
import com.github.forjrking.image.glide.progress.ProgressManager
import com.github.forjrking.image.glide.transformation.BlurTransformation
import com.github.forjrking.image.glide.transformation.BorderTransformation
...
import com.github.forjrking.image.glide.transformation.RoundedCornersTransformation
...
object GlideImageLoader {
    ...
    fun loadImage(options: ImageOptions) {
        ...
        glideRequest.apply {
            ...
            // 缩略图请求
            thumbnail(options.thumbnailUrl?.let { glideRequests(context).load(it) })
...
//            }
            ...
            if (drawableOptions.errorDrawable != null) {
                kotlin.error(drawableOptions.errorDrawable!!)
            } else if (drawableOptions.errorResId != 0) {
                kotlin.error(drawableOptions.errorResId)
            }
            //设置请求 url 为空图片
                ...
                if (options.isCircle) {
                    transform(
                        CircleWithBorderTransformation(
                            options.borderWidth, options.borderColor
                        )
                    )
                } else {
            ...
            if (options.isRoundedCorners) {
                ...
                if (scaleType == ImageView.ScaleType.FIT_CENTER || scaleType == ImageView.ScaleType.CENTER_INSIDE || scaleType == ImageView.ScaleType.CENTER || scaleType == ImageView.ScaleType.CENTER_CROP) {
                    transformation = CenterCrop()
                }
                if (transformation == null) {
                    transform(
                        RoundedCornersTransformation(
                            options.roundRadius, 0, options.cornerType
                        )
                    )
                } else {
                    transform(
                        transformation, RoundedCornersTransformation(options.roundRadius, 0, options.cornerType)
                    )
                }
            }
...
//            into(GlideImageViewTarget(options.imageView, options.res))
    ...
    suspend fun downloadImage(context: Context, imgUrl: String?): File? = withContext(Dispatchers.IO) {
        ...
        MediaScannerConnection.scanFile(
            context, arrayOf(targetFile.absolutePath), arrayOf(mimeTypes), null
        )
        targetFile
    }
}