...
import com.ds.imageload.GlideImageLoader
import com.bumptech.glide.load.Transformation
import com.ds.imageload.ImageOptions
import com.ds.imageload.ImageOptions.DrawableOptions.Companion.DEFAULT
...
@JvmOverloads
fun ImageView.loadImageOverriveSize(
    uri: Uri, overrideSize: ImageOptions.OverrideSize
) {
    this.loadImage(load = uri, size = overrideSize)
}

@JvmOverloads
fun ImageView.loadImageOverriveSize(
    uri: File, overrideSize: ImageOptions.OverrideSize
) {
    this.loadImage(load = uri, size = overrideSize)
}
...
@JvmOverloads
fun ImageView.loadImageOverriveSize(
    uri: Bitmap, overrideSize: ImageOptions.OverrideSize
) {
    this.loadImage(load = uri, size = overrideSize)
}
...
fun ImageView.loadImage(
    ...
    this.loadImage(load = drawableId, placeHolderResId = drawableId)
}
...
fun ImageView.loadImage(
    ...
    this.loadImage(
        load = url, placeHolderResId = placeHolder, errorResId = errorId, requestListener = requestListener
    )
}
...
fun ImageView.loadProgressImage(
    ...
    this.loadImage(
        load = url, placeHolderResId = placeHolder, errorResId = placeHolder, onProgressListener = progressListener
    )
}
...
fun ImageView.loadResizeImage(
    ...
    this.loadImage(
        load = url, placeHolderResId = placeHolder, errorResId = placeHolder, size = ImageOptions.OverrideSize(width, height)
    )
}
...
fun ImageView.loadGrayImage(
    ...
    this.loadImage(
        load = url, placeHolderResId = placeHolder, errorResId = placeHolder, isGray = true
    )
}
...
fun ImageView.loadBlurImage(
    ...
    this.loadImage(
        load = url, placeHolderResId = placeHolder, errorResId = placeHolder, isBlur = true, blurRadius = radius, blurSampling = sampling
    )
}
...
fun ImageView.loadRoundCornerImage(
    ...
    this.loadImage(
        load = url, placeHolderResId = placeHolder, errorResId = placeHolder, isRoundedCorners = radius > 0, roundRadius = radius, cornerType = type
    )
}
...
fun ImageView.loadCircleImage(
    ...
    this.loadImage(
        load = url, placeHolderResId = placeHolder, errorResId = placeHolder, isCircle = true, borderWidth = borderWidth, borderColor = borderColor
    )
}
...
fun ImageView.loadBorderImage(
    ...
    this.loadImage(
        load = url, placeHolderResId = placeHolder, errorResId = placeHolder, borderWidth = borderWidth, borderColor = borderColor
    )
}
...
/**模仿 coil**/
fun ImageView.load(load: Any?, isAnim: Boolean, options: (ImageOptions.() -> Unit)? = null) {
    val imageOptions = ImageOptions().also {
        it.res = load
        it.imageView = this
        it.isAnim = isAnim
...
fun ImageView.load(url: String, thumbnailUrl: String) {
    Glide.with(context).load(url).thumbnail(getimage(context, thumbnailUrl)).into(this)
}
 ...
 * @return
 */
fun getimage(context: Context?, path: String?): RequestBuilder<Drawable?>? {
